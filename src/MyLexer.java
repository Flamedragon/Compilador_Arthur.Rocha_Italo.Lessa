...
import portugol.lexer.Lexer.State;
import portugol.node.*;
public class MyLexer extends Lexer
{ private int count;
  private TBlockCommentStart comment;
  ...
  { // if we are in the comment state
    ...
    { // if we are just entering this state
      ...
      { // The token is supposed to be a comment.
        ...
        comment = (TBlockCommentStart) token;
      ...
      { // we were already in the comment state
        ...
        if(token instanceof TBlockCommentStart)
          count++;
        else if(token instanceof TBlockCommentEnd)
          count--;
        if(count != 0)
          token = null; // continue to scan the input.
        else
